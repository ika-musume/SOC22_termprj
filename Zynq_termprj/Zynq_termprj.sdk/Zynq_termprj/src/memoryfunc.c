...
void copyTilesWithOffset(int charcode, const char data[], int size, int offset) {
    ...
    for(i=0; i<size; i=i+2) {
        chardata = (((unsigned int)data[offset + i] & 0xFF) << 8 ) | ((unsigned int)data[offset + i+1] & 0xFF);
        memWrite(3, addr, chardata);
        addr++;
    }
}
...
void writePalette(int colorcode, int num, unsigned int data) {
    unsigned int addr;
...
void spriteWriteText(char message[], int palette, int xpos, int ypos, int memoffset, int length) {
    ...
    for(i=0; i<length; i++) {
        ...
        if(asciibuf > 0x40 && asciibuf < 0x5B) { //alphabet
            objcode = ((asciibuf - 0x40) * 2) + 0x20;

            spriteAttrWrite(50 + memoffset + i, 4, 0, 0, objcode, palette, xpos, yposAcc, 0x80);
        }
        else if(asciibuf >= 0x30 && asciibuf < 0x3A) { //numbers
            objcode = (asciibuf - 0x30) * 2;

            spriteAttrWrite(50 + memoffset + i, 4, 0, 0, objcode, palette, xpos, yposAcc, 0x80);
        }
        else if(asciibuf == 0x20) {
            spriteAttrWrite(50 + memoffset + i, 4, 0, 0, 0x20, 0, 0, 0, 0x3FF);
        }
